...
@Component({
  selector: 'app-cargar-nota-debito',
  standalone: true,
  imports: [
    EncabezadoComponent,
  ...
  templateUrl: './cargar-nota-debito.component.html',
  styleUrl: './cargar-nota-debito.component.scss'
})
export class CargarNotaDebitoComponent {
  formularioCargarNotaDebito : FormGroup;
  listaClientes : any[] = [];
  valoresFiltradosClientes : any[] = [];
  datosSubtotalNotaDebito : any = {
    subtotal: 0.0,
    IVA: 0.0,
    subtotalIVA: 0.0,
    totalNeto: 0.0
  ...
  constructor(
    ...
    this.formularioCargarNotaDebito = this.formBuilder.group({
  ...
  ngOnInit() : void {
    ...
    this.formularioCargarNotaDebito.valueChanges.subscribe(() => {
      this.recalcularResultadoNotaDebito();
    })
  }
  
  get items() {
    return this.formularioCargarNotaDebito.get('items') as FormArray;
  }

  get comprobantesAsociados() {
    return this.formularioCargarNotaDebito.get('comprobantesAsociados') as FormArray;
  }
  ...
  obtenerListaClientes() : void {
   ...
   recalcularResultadoNotaDebito() {
    let subtotalNotaDebito = 0.0;
    
    this.items.controls.forEach(item => {
      subtotalNotaDebito += parseFloat(item.get('importe')?.value || 0) * ( 1 + parseFloat(item.get('iva')?.value || 0) / 100);
    });

    this.datosSubtotalNotaDebito.subtotal = subtotalNotaDebito;
   }
  ...
  obtenerUltimoCae() {
    const datosCliente = this.listaClientes.find(cliente => cliente.clientName == this.formularioCargarNotaDebito.get('cliente')?.value);
    const puntoVenta = this.formularioCargarNotaDebito.get('puntoVenta')?.value;
    ...
    if(datosCliente.ivaType == 'RESPONSABLE INSCRIPTO') {
      datosEnvio.comprobante = 'NOTA DE DÉBITO A';
    } else if(datosCliente.ivaType == 'MONOTRIBUTO' || datosCliente.ivaType == 'CONSUMIDOR FINAL') {
      datosEnvio.comprobante = 'NOTA DE DÉBITO B';
    }
    ...
    this._peticionesHttp.obtenerUltimoCAE(datosEnvio).subscribe({
      next: (data) => {
        ...
        this.formularioCargarNotaDebito.patchValue({
   ...
   cargarNotaDebito() {
    if(!this.formularioCargarNotaDebito.valid) {
      return;
    }

    let comprobantesAsociadosNotaDebito : any[] = [];
    let itemsNotaDebito : any[] = [];
    ...
    this.comprobantesAsociados.controls.forEach(comprobante => {
      ...
      comprobantesAsociadosNotaDebito.push({
    ...
    let data = {
      cae: this.formularioCargarNotaDebito.value.cae,
      descuento: this.formularioCargarNotaDebito.value.descuento,
      cliente: this.formularioCargarNotaDebito.value.cliente,
      fecha: this.formularioCargarNotaDebito.value.fecha,
      puntoVenta: this.formularioCargarNotaDebito.value.puntoVenta,
      idCliente: this.obtenerIdClienteSegunNombre(),
      observaciones: this.formularioCargarNotaDebito.value.observaciones,
      comprobantesAsociados: comprobantesAsociadosNotaDebito,
      itemsNotaDebito: itemsNotaDebito,
      resultadoFactura: this.datosSubtotalNotaDebito
    };
    
    this._peticionesHttp.cargarNotaDebito(data).subscribe({
      next: (data) => {
        if(!data.error) {
          this.formularioCargarNotaDebito.reset();
        }
   ...
   obtenerIdClienteSegunNombre() : number {
     ...
     const datosCliente = this.listaClientes.find(cliente => cliente.clientName.toLowerCase() === this.formularioCargarNotaDebito.value.cliente.toLowerCase());
